...
def render_reference() -> None:
    # Формулы 
    st.subheader("Формулы расчета ИПР")
...
""")
        ...
        st.markdown("**Формула (8) - учёт противопожарных дверей (п. 48)** *(не реализована)*:")
        st.latex(
            r"R_i = P_{\text{откр}} \cdot R_i^{\text{откр}} + P_{\text{закр}} \cdot R_i^{\text{закр}}, "
            r"\quad P_{\text{откр}} = 0{,}3,\ P_{\text{закр}} = 0{,}7"
        )
        st.caption(
            "Для учёта противопожарных дверей создайте отдельные сценарии "
            "с разными значениями tбл (дверь открыта / дверь закрыта)."
        )
    ...
    # Справочные таблицы 
    st.subheader("Справочные таблицы")
