...
def render_scenarios() -> None:
    ...
    st.caption(
        ...
        "коэффициент АУП Kап (п. 15), и параметры CПС, СОУЭ и ПДЗ (п. 41, 44, 45 соответственно)."
    )
    st.info(
        "Время блокирования tбл определяется расчётом по Приложению 6 с помощью аналитического подхода (методом критической зоны)"
