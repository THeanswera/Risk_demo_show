...
def test_compute_all_k_ap_from_checkbox(default_scenarios, default_groups):
    """K_ап правильно вычисляется из чекбокса (п. 15 Методики №1140)."""
    df_scen, _, _, _ = compute_all(default_scenarios, default_groups)
    assert "K_ап,i" in df_scen.columns
    assert df_scen["K_ап,i"].iloc[0] == pytest.approx(0.9)
...
def test_compute_all_high_risk_scenario():
    ...
    scen = pd.DataFrame([{
        ...
        "t_пр,i (ч/сут)": 24.0,
        "t_бл,i (мин)": 5.0,
        "АУП соответствует? (K_ап=0.9)": False,
